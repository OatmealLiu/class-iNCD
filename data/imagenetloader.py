...
def make_dataset_30(dir, classes, class_to_idx):
    samples = []
    for target in classes:
        d = os.path.join(dir, target)
        if not os.path.isdir(d):
        ...
        for root, _, fnames in sorted(os.walk(d)):
            for fname in sorted(fnames):
                path = os.path.join(root, fname)
                item = (path, class_to_idx[target]+882)
                if 'JPEG' in path or 'jpg' in path:
                    samples.append(item)
...
def ImageNetConcat(aug=None, subfolder='train', class_list=range(882), path='./data/datasets/ImageNet/', subset='A'):
    ...
    classes_concat = list(set(classes_882) | set(classes_30))
    class_to_idx_concat = {classes_concat[i]: i for i in range(len(classes_concat))}

    # class_to_idx_882 = {classes_882[i]: i for i in range(len(classes_882))}
    classes_sel = [classes_concat[i] for i in class_list]

    samples = make_dataset(path+img_split, classes_sel, class_to_idx_concat)
...
def ImageNet882(aug=None, subfolder='train', path='./data/datasets/ImageNet/'):
    ...
    # with open(os.path.join(path, 'imagenet_rand118/imagenet_882.txt'), 'w') as f:
    #     for line in classes_882:
    #         f.write(line)
    #         f.write('\n')

    classes_882, class_to_idx_882 = find_classes_from_file(os.path.join(path, 'imagenet_rand118/imagenet_882.txt'))
...
def ImageNet30(path='./data/datasets/ImageNet/', subset='A', aug=None, subfolder='train'):
    classes_30, class_to_idx_30 = find_classes_from_file(os.path.join(path, 'imagenet_rand118/imagenet_30_{}.txt'.format(subset)))
    samples_30 = make_dataset_30(path+'images/{}'.format(subfolder), classes_30, class_to_idx_30)
...
def ImageNetLoader882_30_Concat(batch_size, num_workers=2, path='./data/datasets/ImageNet/',  unlabeled_subset='A', aug=None, shuffle=False, subfolder='train'):
    dataset_labeled = ImageNet882(aug=aug, subfolder=subfolder, path=path)
    dataset_unlabeled = ImageNet30(path, unlabeled_subset, aug, subfolder)
    dataset = ConcatDataset((dataset_labeled, dataset_unlabeled))
    # labeled_idxs = range(len(dataset_labeled))
...
def ImageNetLoader(batch_size, num_workers=2, path='./data/datasets/ImageNet/', unlabeled_subset='A', aug=None, shuffle=False, class_list=range(882), subfolder='train'):
    dataset = ImageNetConcat(aug=aug, subfolder=subfolder, class_list=class_list, path=path, subset=unlabeled_subset)
    dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True)
    return dataloader
